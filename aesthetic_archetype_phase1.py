...
class AestheticArchetype(str, Enum):
    MINIMAL = "minimal"
    CLASSIC_POLISHED = "classic_polished"
    QUIET_LUXURY = "quiet_luxury"
    ROMANTIC_FEMININE = "romantic_feminine"
    BOHEMIAN = "bohemian"
    PREPPY = "preppy"
    ...
    ECLECTIC_MAXIMALIST = "eclectic_maximalist"


ALLOWED_LABELS = [member.value for member in AestheticArchetype]
ALLOWED_LABELS_SET = set(ALLOWED_LABELS)


class ArchetypeChoice(BaseModel):
    label: AestheticArchetype
    confidence: float = Field(..., ge=0.0, le=1.0)


class AestheticArchetypeResult(BaseModel):
    archetypes: list[ArchetypeChoice] = Field(..., min_length=2, max_length=2)

    @model_validator(mode="after")
    def validate_distinct_labels(self) -> "AestheticArchetypeResult":
        labels = [item.label.value for item in self.archetypes]
        if len(set(labels)) != 2:
            raise ValueError("archetypes must contain exactly 2 distinct labels")
        return self


AESTHETIC_SCHEMA = AestheticArchetypeResult.model_json_schema()


SYSTEM_PROMPT = """
You are a strict fashion aesthetic-archetype classifier.
You must output valid JSON only.
You must choose exactly 2 DISTINCT labels from the provided schema enum.
...
Classify the SINGLE MAIN FASHION PRODUCT in the featured image into exactly 2 DISTINCT aesthetic archetypes.
...
- Pick the 2 strongest-fitting archetypes.
- The 2 labels must be distinct.
- Confidence must be a float between 0.0 and 1.0 for each label.
- Prefer visually grounded style signals such as silhouette, cut, material impression, styling language, color story, details, and overall aesthetic vibe.
...
def canonicalize_label(label: str) -> str:
    ...
    alias_map = {
        "classic": "classic_polished",
        "classicpolished": "classic_polished",
        "quietluxury": "quiet_luxury",
        "romantic": "romantic_feminine",
        "romanticfeminine": "romantic_feminine",
        "vintage": "vintage_retro",
        "retro": "vintage_retro",
        "eclectic": "eclectic_maximalist",
        "maximalist": "eclectic_maximalist",
        "sporty_athleisure": "sporty",
    }
    return alias_map.get(v, v)


def normalize_result_payload(raw_obj: dict) -> dict:
    if isinstance(raw_obj, dict) and isinstance(raw_obj.get("archetypes"), list):
        for item in raw_obj["archetypes"]:
            if isinstance(item, dict) and isinstance(item.get("label"), str):
                item["label"] = canonicalize_label(item["label"])
        return raw_obj

    # permissive fallback if the model returns first/second style keys
    fallback = []
    for key in ("first", "primary", "first_archetype"):
        if key in raw_obj and isinstance(raw_obj[key], dict):
            fallback.append(raw_obj[key])
            break
    for key in ("second", "secondary", "second_archetype"):
        if key in raw_obj and isinstance(raw_obj[key], dict):
            fallback.append(raw_obj[key])
            break
    ...
    raise ValueError("Response JSON does not contain expected archetypes payload")
...
DELETE_EXISTING_SQL = "DELETE FROM aesthetic_archetypes WHERE product_id = %s"
INSERT_SQL = """
INSERT INTO aesthetic_archetypes (product_id, label, confidence, source_run_id)
VALUES (%s, %s, %s, %s)
"""
...
def classify_one(
    ...
    for attempt in range(1, api_max_attempts + 1):
        try:
            ...
            parsed = AestheticArchetypeResult.model_validate(normalized)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aesthetic archetype classifier via local vLLM vision model"
    )
    ...
    parser.add_argument("--work-dir", default="./aesthetic_archetype_phase1_work")
    parser.add_argument("--log-name", default="aesthetic_archetype_phase1")
    ...
    if args.resume:
        ...
        source_run_id = f"aesthetic_archetype_phase1_{utc_now_compact()}"
        state = {
            ...
            while state["attempted_count"] < args.max_products:
                ...
                for future in as_completed(futures):
                    ...
                    if result["ok"]:
                        ...
                        logger.info(
                            "product_id=%s title=%s archetypes=%s",
                            result["product_id"],
                            result["title"],
                            ", ".join(
                                f"{item['label']} ({item['confidence']:.2f})" for item in result["archetypes"]
                            ),
                        )
